...
contract Web3University is Ownable {
    ...
    event DDTokenUpdated(address indexed newDDToken);

    // 无参构造函数，使用 msg.sender 作为初始所有者
    constructor() Ownable(msg.sender) {
        // 可以在部署后通过 updateDDToken 设置代币地址
    }
    ...
    function createCourse(
        address _author,
        uint256 _price
    ) external onlyOwner validDDToken returns (uint256) {
    ...
    function purchaseCourse(
        uint256 _courseId
    ) external validCourse(_courseId) validDDToken {
        require(
     ...
     * 作者提现收益（扣除平台费用）
     */
    function withdrawAuthorEarnings(uint256 _courseId) external validDDToken {
        require(
            _courseId > 0 && _courseId <= totalCourses,
            "Invalid course ID"
        );
     ...
     * 平台提现（只有所有者可以调用）
     */
    function withdrawPlatformFees() external onlyOwner validDDToken {
    ...
    function updateDDToken(address _newDDToken) external onlyOwner {
        ...
        emit DDTokenUpdated(_newDDToken);
    }

    /**
     * 紧急提现功能（只有所有者可以调用）
     */
    function emergencyWithdraw() external onlyOwner validDDToken {
        uint256 balance = ddToken.balanceOf(address(this));
        if (balance > 0) {
            require(ddToken.transfer(owner(), balance), "Transfer failed");
        }
